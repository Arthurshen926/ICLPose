#!/usr/bin/env python3
"""Train Stage C2.16 diagonal descriptor channel selection."""
...
from feature_extract.vfm.confidence_descriptor_refinement import (
    ...
    save_diagonal_descriptor_selector_checkpoint,
            ...
            TokenBankRecord(
                ...
                layers=(
                    TokenLayerSpec(
                        ...
                        model=f"{spec.model}:stage_c216_diagonal_selector",
                        layer=spec.layer,
                ...
                metadata={
                    **dict(record.metadata),
                    "stage": "stage_c216_diagonal_descriptor_selection",
...
def _transform_track_bank(source_bank: Path, output_bank: Path, run, device: str, batch_rows: int) -> dict[str, object]:
    ...
    return {
        ...
        "variance_policy": "zero_for_diagonal_descriptor_selection_export",
    }
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    parser.add_argument("--distill_loss_weight", type=float, default=0.1)
    parser.add_argument("--anchor_loss_weight", type=float, default=0.05)
    parser.add_argument("--scale_regularization_weight", type=float, default=0.01)
    parser.add_argument("--max_log_scale", type=float, default=0.5)
    ...
    run = train_diagonal_descriptor_selector(
        ...
        DiagonalDescriptorSelectionConfig(
            ...
            scale_regularization_weight=float(args.scale_regularization_weight),
    ...
    save_diagonal_descriptor_selector_checkpoint(run, args.output_model)
