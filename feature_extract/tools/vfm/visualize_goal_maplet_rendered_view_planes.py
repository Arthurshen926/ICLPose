...
def main():
 p=argparse.ArgumentParser();p.add_argument('--physical_map',type=Path,required=True);p.add_argument('--contributor',type=Path,required=True);p.add_argument('--output',type=Path,required=True);a=p.parse_args();table=PrimitiveSurfaceTable.from_physical_map(GoalMapletPhysicalMap.load_npz(a.physical_map));o=extract_rendered_plane_observations(table,a.contributor)
