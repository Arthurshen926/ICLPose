...
def main():
 ...
 with np.load(b/'native_context_v278/map.npz') as z:source_names=set(json.loads(z['metadata_json'].item())['offline_mapping_source_names'])
 ...
 sources={str(f):file_sha256(f) for f in [initial,mp,*cp,Path(__file__),Path(__file__).with_name('direct_anonymous_feature_pose.py')]};sources.update({str(Path(__file__).with_name('directional_feature_memory.py')):file_sha256(Path(__file__).with_name('directional_feature_memory.py'))} if a.directional else {});out={k:[] for k in arms};audit={k:[] for k in arms};start=time.perf_counter()
 for i,n in enumerate(poses['names'].astype(str)):
  ...
  if a.visibility:
   ...
   rows=rows[vm]
  cache=b/'native_fine_v264/query_cache'/n;grids,checkpoint=load_grids(cache,mm['projection_sha256']);sources[str(cache)]=file_sha256(cache)
