...
parser=argparse.ArgumentParser();parser.add_argument('--output',type=Path);parser.add_argument('--bank-root',type=Path);args=parser.parse_args()
mp=b/'native_fine_v264/readout/map.npz';map_hash=file_sha256(mp)
with np.load(mp) as f:world=f['world_points'];map_meta=json.loads(f['metadata_json'].item())
atlas_path=b/'stmarys_metric_plane_uv_radio_atlas_cell050_p4_learned64d_strict_v9.npz'
if file_sha256(atlas_path)!=map_meta['atlas_sha256']:raise ValueError('atlas/fine map binding mismatch')
for label,refroot,arm in [('mnn_seed1','overlap_lod_v402/lod_fixed_final','mnn'),('mnn_seed2','overlap_lod_v402/lod_seed2_final','mnn'),('soft_seed1','overlap_weighted_v403/lod_final','weighted_mnn'),('soft_seed2','overlap_weighted_seed2_v403/lod_final','weighted_mnn')]:
 out=(args.output or r)/label;out.mkdir(exist_ok=True,parents=True)
 for s in ['seq10','shard0','shard1','shard2','shard3']:
  bank=(args.bank_root or r/'banks')/(s+'.npz');old=previous/(label+'_pairwise')/f'{s}_verified.npz';audit=json.load(open(previous/(label+'_pairwise')/f'{s}_audit.json'));bp=b/'regularized_stage_precision_v357'/f'{s}_stage_plain_reg_all.npz'
  with np.load(bank) as f:bk={k:f[k] for k in f.files if k!='metadata_json'};bank_meta=json.loads(f['metadata_json'].item())
  ...
  with np.load(cp) as f:validate_bank(bk,bank_meta,world,mp,map_hash,map_meta['atlas_sha256'],names,f['camera_matrices'],f['radial_k1'])
  validate_poses(base);validate_poses(proposed)
  values={k:[] for k in ['veto','corroborate','threshold']};accepts={k:[] for k in values};details=[]
