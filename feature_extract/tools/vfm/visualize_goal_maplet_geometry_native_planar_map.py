...
from feature_extract.vfm.localization_goal_maplet.geometry_native_planar_map import GeometryNativePlanarMap,PrimitiveSurfaceTable
...
def main() -> None:
    ...
    parser.add_argument('--scene_title',default='2DGS')
    args=parser.parse_args()
    table=PrimitiveSurfaceTable.load_npz(args.surface_table) if args.surface_table is not None else PrimitiveSurfaceTable.from_physical_map(GoalMapletPhysicalMap.load_npz(args.physical_map))
    planar=GeometryNativePlanarMap.load_npz(args.planar_map,primitive_count=table.primitive_ids.size)
    owner=np.full(table.primitive_ids.size,-1,np.int64)
    ...
    selected=owner>=0
    color=np.full((table.primitive_ids.size,4),[.72,.72,.72,.08])
    ...
    sample=np.sort(rng.choice(table.primitive_ids.size,min(180000,table.primitive_ids.size),replace=False))
    p=table.centers[sample]
    ...
    stats={
        'planes':int(planar.plane_ids.size),'assigned_fraction':float(planar.member_primitive_rows.size/table.primitive_ids.size),
    ...
    fig.suptitle(str(args.scene_title)+' 2DGS → rendered-view fused planar map\n'+json.dumps(stats),fontsize=12)
