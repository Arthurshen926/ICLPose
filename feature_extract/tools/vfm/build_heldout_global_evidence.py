...
p=argparse.ArgumentParser();p.add_argument('--split',required=True);p.add_argument('--mapping',action='store_true');a=p.parse_args();s=a.split;b=Path('output/g25_pose_transport/planar_map_rendered_ransac_v1/surface_coordinate_upgrade_v1');o=b/'heldout_evidence_v405/banks';o.mkdir(exist_ok=True,parents=True);dest=o/(('mapping_' if a.mapping else '')+s+'.npz');assert not dest.exists();maps,meta,sources=load_map(b)
