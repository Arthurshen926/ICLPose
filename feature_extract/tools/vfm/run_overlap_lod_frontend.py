...
def make_matcher(checkpoint):
    if 'shared_physical_identity' in checkpoint['metadata']:
        from feature_extract.tools.vfm.train_shared_identity_matcher import DecoupledMatcher
        return DecoupledMatcher(shared=checkpoint['metadata']['shared_physical_identity'],appearance_only=checkpoint['metadata'].get('appearance_only',False))
    return PartialOverlapMatcher()
