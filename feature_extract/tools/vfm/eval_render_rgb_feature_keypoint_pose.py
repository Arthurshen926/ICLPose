...
def _match_table_rows_for_query(
    *,
    ...
    for idx, match in enumerate(matches):
        ...
        render_xy = np.asarray(render_xy_by_match.get(int(match.track_id), np.full((2,), np.nan)), dtype=np.float64).reshape(2)
...
def _run_render_pose_residual_diagnostic(
    *,
    ...
    oracle_matches = [
        match
        for idx, match in enumerate(matches)
        if idx < errors.shape[0] and np.isfinite(errors[idx]) and errors[idx] <= float(oracle_threshold_px)
    ]
    output["residual_solver_oracle16_match_count"] = int(len(oracle_matches))
    if len(oracle_matches) >= 3:
        try:
            ...
            output.update(
                {
                    "residual_solver_oracle16_translation_error_m": oracle_t,
                    "residual_solver_oracle16_rotation_error_deg": oracle_r,
                }
            )
        except Exception as exc:
            output["residual_solver_oracle16_error"] = str(exc)
    return output
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--matcha_eval_preset", default="none", choices=("none", "conservative_confidence"))
    ...
    parser.add_argument("--render_pose_mode", default="gt", choices=("gt", "gt_offset", "reference_top1", "reference_top5"))
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    reference_top1 = {}
    reference_topk = {}
    if str(args.render_pose_mode) in {"reference_top1", "reference_top5"}:
    ...
    for vis_idx, record in enumerate(records):
        ...
        if bool(args.extract_query_features_from_image):
            query_cache_path = (
                ...
                else _render_token_cache_path(query_cache_dir, record.image_id, int(camera.width), int(camera.height))
            )
        ...
        try:
            if str(args.render_pose_mode) == "reference_top5":
        ...
        topk_selection_mode = "best_score" if str(args.render_pose_mode) == "reference_top5" else str(args.pose_update_selection)
