...
def main():
 p=argparse.ArgumentParser();p.add_argument('--base',type=Path,required=True);p.add_argument('--output',type=Path,required=True);p.add_argument('--input_manifest',type=Path);p.add_argument('--query-cache',type=Path);p.add_argument('--protocol-reference',type=Path);p.add_argument('--crossroute-library-root',type=Path);p.add_argument('--arms',nargs='+',choices=['reliability_mean','reliability_both','reliability_full','reliability_uniform32','reliability_rank32','fixed_full']);a=p.parse_args();b=a.base;o=a.output;o.mkdir(parents=True,exist_ok=False);modelpath=b/'native_reliability_v274/model.json';model=json.load(open(modelpath));mapfile=b/'native_fine_v264/readout/map.npz'
 ...
 for split in (list(inputs) if inputs else ['seq10','shard0','shard1','shard2','shard3']):
  cp=Path(inputs[split]['correspondences']) if inputs else b/'native_scope_v254/wide'/f'{split}_appearance.npz';corr,coarse_meta=_load(cp)
  ...
  exclusion=None
  if a.crossroute_library_root:
   from feature_extract.tools.vfm.refinement_source_contract import validate_crossroute_support
   lineage=b/'native_fine_v264/mapping_lineage.npz';library=a.crossroute_library_root/f'{split}_map.npz'
   if file_sha256(lineage)!=mm['lineage_sha256']:raise ValueError('native lineage differs')
   with np.load(library) as z:members=z['geometry_member_rows'];lm=json.loads(z['metadata_json'].item())
   if lm['native_atlas_sha256']!=mm['atlas_sha256'] or lm['excluded_mapping_route']!=split:raise ValueError('mapping library binding differs')
   with np.load(lineage) as z:exclusion=validate_crossroute_support(corr['names'],corr['prototype_atlas_row'],members,z['prototype_source_and_cell'][:,0],z['source_names'],split)
  initial=Path(inputs[split]['initial_pose']) if inputs else b/'native_global_proposals_v258'/(split+'_token')/'moge.npz';poses,_=_load_frozen_poses(initial);variants={arm:{k:v.copy() for k,v in corr.items()} for arm in arms};audit=[]
  if inputs:
  ...
  for i,name in enumerate(corr['names'].astype(str)):
   ...
   grids,ck=load_grids((a.query_cache or b/'native_fine_v264/query_cache')/name,mm['projection_sha256'])
