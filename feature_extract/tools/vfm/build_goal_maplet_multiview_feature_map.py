"""Bake anonymous descriptors at fixed anchors from visible mapping cameras only."""
...
def main():
    ...
    for j,src in enumerate(sorted(requests)):
        ...
        if len(rows) and cp.exists():
            ...
            for level,grid in enumerate(grids):sums[level,rows]+=sample_grid(grid,xy)
            count[rows]+=1
