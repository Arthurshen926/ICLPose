"""Score fixed candidates using precomputed token descriptor banks."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Score candidates with descriptor cosine similarity")
    parser.add_argument("--bank", required=True)
    parser.add_argument("--query_descriptors", required=True)
    parser.add_argument("--map_descriptors", required=True)
    parser.add_argument("--method", default="raw_radio_descriptor_cosine")
    ...
    if args.output_md:
        ...
        output_md.write_text(build_gate_table("Descriptor Cosine Baseline", [report]) + "\n")
