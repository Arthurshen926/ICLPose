...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--score-mode", choices=("global", "local"), default="global")
...
@torch.no_grad()
def evaluate_bucket(model, loader, map_renderer, cfg: Dict, args: argparse.Namespace, bucket) -> Dict:
    ...
    for batch in loader:
        ...
        scores, combo_valid = score_combo(teacher_query, map_render, candidate_pose, valid, args)
        append_combo_metrics(rows, "teacher_query_vs_map_base", scores, pose_cost, combo_valid, trans_err, rot_err, args)

        scores, combo_valid = score_combo(student_query, map_render, candidate_pose, valid, args)
        ...
        if projector is not None:
            ...
            if used_projector:
                ...
                scores, combo_valid = score_combo(query_proj, render_proj, candidate_pose, valid, args)
...
def main() -> None:
    ...
    for bucket in parse_buckets(args.buckets):
        ...
        results[bucket[0]] = evaluate_bucket(model, loader, map_renderer, cfg, args, bucket)
