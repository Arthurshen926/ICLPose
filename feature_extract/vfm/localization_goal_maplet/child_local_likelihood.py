...
def predict_child_local_surface_likelihood(
    ...
    *,
    ...
    for index, child in enumerate(children.tolist()):
        ...
        coverage[index] = float(np.sum(membership[valid]) / total_membership)
        if not np.any(valid):
        ...
        similarity = np.asarray(field.codes[selected] @ query[index], dtype=np.float64)
        reliability = np.maximum(
            field.confidence[selected] * (1.0 - field.uncertainty[selected]), 1e-4
        )
        logits = similarity / float(temperature)
        logits += float(confidence_prior_weight) * np.log(reliability)
        logits += 0.10 * np.log(np.maximum(membership, 1e-6))
        if geometry_conditioned:
            ...
            residual = np.linalg.norm(projected - xy_px[index], axis=1)
            sigma = max(float(scale_px[index]), 8.0)
            logits += float(geometry_weight) * -0.5 * np.square(residual / sigma)
            logits[(camera_xyz[:, 2] <= 0.05) | ~front] = -1e6
        logits -= float(np.max(logits))
        probability = np.exp(np.clip(logits, -60.0, 0.0))
        probability /= max(float(np.sum(probability)), 1e-12)
        order = np.argsort(-probability, kind="stable")[:mode_count]
        mode_rows[index, : order.size] = primitive[order]
        mode_probability[index, : order.size] = probability[order]
        map_points[index] = physical.primitive_centers[primitive[int(order[0])]]
        expected_points[index] = np.sum(
            probability[:, None] * physical.primitive_centers[primitive], axis=0
        )
        local = (
            physical.primitive_centers[primitive] - expected_points[index]
        ) @ physical.child_frames[child].T
        covariance[index] = np.einsum("n,ni,nj->ij", probability, local[:, :2], local[:, :2])
        maximum_similarity[index] = float(np.max(similarity))
        entropy[index] = float(-np.sum(probability * np.log(np.maximum(probability, 1e-12))))
        ...
        null[index] = float(np.clip(1.0 - coverage[index], 0.0, 1.0))
