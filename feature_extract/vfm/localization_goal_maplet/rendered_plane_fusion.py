...
def fuse_rendered_plane_observations(
 ...
 metadata={'artifact_type':SCHEMA,'representation':'finite_planes_from_2dgs_rendered_depth_sequential_ransac_cross_view_fusion','uses_parent_child_partition':False,'uses_voxel_identity_or_boundary':False,'uses_query_pose_or_ground_truth':False,'uses_mapping_rgb':False,'primitive_count':int(table.primitive_ids.size),'plane_count':count,'assigned_primitive_count':int(arrays['member_primitive_rows'].size),'observation_count':global_observation,'configuration':{'normal_degrees':normal_degrees,'offset_m':offset_m,'maximum_aggregate_rms_m':maximum_aggregate_rms_m,'minimum_members':minimum_members,'minimum_views':minimum_views},'boundary':'convex_summary_plus_exact_member_primitive_inventory','boundary_role':'visualization_and_broad_phase_only','finite_support_authority':'exact_member_primitive_rows','boundary_uv_localization_eligible':False}
