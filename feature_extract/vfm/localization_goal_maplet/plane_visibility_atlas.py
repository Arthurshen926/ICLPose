...
class PlaneVisibilityAtlas:
 ...
 def validated(self):
  ...
  if a['view_names'].shape!=(n,) or a['poses_w2c'].shape!=(n,4,4) or a['centers_world'].shape!=(n,3) or a['token_pixel_counts'].shape!=(n,36,64):raise ValueError('invalid visibility atlas shapes')
  if a['token_pixel_counts'].dtype!=np.uint8 or np.any(a['token_pixel_counts']>16):raise ValueError('invalid 4x4 token counts')
...
def build_plane_visibility_atlas(observation_paths,lineage_offsets,lineage_rows,contributors_dir):
 ...
 for path in paths:
  ...
  for local in range(len(observation.normals_world)):
   mask=(observation.labels==local).astype(np.uint8);counts=mask.reshape(36,4,64,4).sum((1,3)).astype(np.uint8);lookup[global_row]=(meta['source_name'],pose,center,counts);global_row+=1
...
__all__=['PlaneVisibilityAtlas','build_plane_visibility_atlas','mask_dice']
