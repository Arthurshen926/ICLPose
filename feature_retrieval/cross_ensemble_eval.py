#!/usr/bin/env python3
"""
Cross-dimension ensemble evaluation: combine models trained on different 
feature dimensions (e.g., 64d and 128d PCA features).
"""
import argparse
import json
import math
...
def load_model_and_predict(ckpt_path, feature_dir, dataset_dir, device):
    """Load a model checkpoint and generate predictions using correct features."""
    ...
    # Load test data with correct features
...
def main():
    parser = argparse.ArgumentParser(description='Cross-Dimension Ensemble Evaluation')
    parser.add_argument('--models', nargs='+', required=True,
                        help='model_dir:feature_dir pairs (e.g., path/to/model:path/to/features)')
    ...
    # Default feature dirs
    FEAT_64 = 'output/feature_extract/features_radio_dual/OldHospital_pilot'
    FEAT_128 = 'output/feature_extract/features_radio_dual_128/OldHospital_pilot'
    
    # Load train data for retrieval eval (using 64d, just for positions)
    print("Loading train data (for retrieval evaluation)...")
    train_data = PatchPoseDataset(
        FEAT_64, args.dataset_dir, 'train', 'cpu',
    ...
    for model_spec in args.models:
        if ':' in model_spec:
            model_dir, feature_dir = model_spec.split(':', 1)
        else:
            # Auto-detect: check config for patch_dim
            ...
            if config_path.exists():
                ...
                feature_dir = FEAT_128 if patch_dim == 128 else FEAT_64
            else:
                feature_dir = FEAT_64
        ...
        print(f"  Loading: {model_dir} (features: {'128d' if '128' in feature_dir else '64d'})")
    ...
    results = evaluate_predictions(ens_trans, ens_rot, test_data_ref, train_data)
