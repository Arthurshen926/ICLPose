#!/usr/bin/env python3
"""Evaluate MSFlowPoseNet with variable outer/inner iteration sweeps.

Sweeps: outer_iters, fine_iters, mid_iters, irls_iters — all inference-time.

Usage:
    python scripts/eval_hyperparams.py --config configs/exp101_stairs_oi10.yaml \
        --checkpoint output/exp101_stairs_oi10/checkpoints/best.pth \
        --gpu 1 --outer-iters 5 10 15 20 \
        --fine-iters 8 12 16 --mid-iters 1 3 --irls-iters 3 5
"""
...
def eval_one(model, renderer, val_loader, device, outer_iters):
    all_rot, all_trans = [], []
    with torch.no_grad():
        for batch in tqdm(val_loader, desc=f"oi={outer_iters:2d}", leave=False):
    ...
    return {
        'rot_mean': np.mean(r), 'rot_med': np.median(r),
        'trans_mean': np.mean(t), 'trans_med': np.median(t),
        'lt1': np.mean(r < 1) * 100, 'lt05': np.mean(r < 0.5) * 100,
    }
...
def main():
    ...
    parser.add_argument('--outer-iters', type=int, nargs='+', default=[5, 10, 15, 20])
    parser.add_argument('--fine-iters', type=int, nargs='+', default=None,
                        help='Sweep fine GRU iterations (default: use config value)')
    parser.add_argument('--mid-iters', type=int, nargs='+', default=None,
                        help='Sweep mid iterations (default: use config value)')
    parser.add_argument('--irls-iters', type=int, nargs='+', default=None,
                        help='Sweep IRLS iterations (default: use config value)')
    ...
    print(f"[Loading renderer...] intrinsics: {intrinsics['fx']:.1f}/{intrinsics['img_height']}x{intrinsics['img_width']}")
    renderer = MultiScaleRenderer(
        ply_path=rc['ply_path'], scale_model_paths=rc['scale_model_paths'],
        device=device, **intrinsics)

    mc = cfg['model']
    base_fine_iters = mc.get('fine_iters', 8)
    base_mid_iters = mc.get('mid_iters', 1)
    base_irls_iters = mc.get('irls_iters', 3)

    fine_iters_list = args.fine_iters or [base_fine_iters]
    mid_iters_list = args.mid_iters or [base_mid_iters]
    irls_iters_list = args.irls_iters or [base_irls_iters]

    # Intrinsics dict for model (must be dict with float values)
    model_intrinsics = {
        'fx': float(intrinsics['fx']), 'fy': float(intrinsics['fy']),
        'cx': float(intrinsics['cx']), 'cy': float(intrinsics['cy']),
    }
    img_hw = (intrinsics['img_height'], intrinsics['img_width'])
    ...
    for fi in fine_iters_list:
        for mi in mid_iters_list:
            for ir in irls_iters_list:
                # Create model with these hyperparams
                model = MSFlowPoseNet(
                    ...
                    irls_iters=ir, intrinsics=model_intrinsics, img_hw=img_hw,
                ).to(device)
                model.load_state_dict(ckpt['model_state_dict'])
                model.eval()

                header = f"fine_iters={fi}, mid_iters={mi}, irls_iters={ir}"
                print(f"\n  [{header}]")

                for oi in args.outer_iters:
                    m = eval_one(model, renderer, val_loader, device, oi)
                    tag = f"fi={fi},mi={mi},ir={ir},oi={oi}"
                    results.append((tag, m))
                    print(f"    oi={oi:2d}: rot={m['rot_mean']:.3f}° (med {m['rot_med']:.3f}°)  "
                          f"trans={m['trans_mean']:.1f}mm (med {m['trans_med']:.1f}mm)  "
                          f"<1°={m['lt1']:.1f}%  <0.5°={m['lt05']:.1f}%")

                del model
                torch.cuda.empty_cache()

    # Summary — sort by median rotation
    print(f"\n{'='*80}")
    print("  SUMMARY (sorted by median rotation)")
    print(f"{'='*80}")
    results.sort(key=lambda x: x[1]['rot_med'])
    for tag, m in results[:20]:
        print(f"  {tag:30s} → rot_med={m['rot_med']:.3f}° trans_med={m['trans_med']:.1f}mm  "
              f"<1°={m['lt1']:.1f}%  <0.5°={m['lt05']:.1f}%")
