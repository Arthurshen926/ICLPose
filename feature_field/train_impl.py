...
def train(cfg, resume_path=None):
    ...
    if refiner_type == 'depth_guided':
        ...
        feat_sharp_fine = DepthGuidedRefiner(
            mcfg['feature_dim'], hidden_dim=refiner_hdim,
        ).cuda()
    ...
    if warmstart_path and os.path.exists(warmstart_path):
        ...
        if 'projection_state' in warm_ckpt and teacher is not None:
            teacher.proj_fine.load_state_dict(warm_ckpt['projection_state']['fine'])
            teacher.proj_coarse.load_state_dict(warm_ckpt['projection_state']['coarse'])
    ...
    for iteration in range(start_iteration + 1, total_iters + 1):
        ...
        is_best = False
        if iteration >= coarse_save_start and losses['total'].item() < best_metrics['total_loss']:
            is_best = True
        ...
        if is_best and iteration >= coarse_save_start:
            best_ckpt = {
                'iteration': iteration,
                'config': cfg,
