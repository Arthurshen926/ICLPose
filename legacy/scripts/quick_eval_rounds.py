#!/usr/bin/env python3
"""
Quick eval: test different round counts on 30 samples to find optimal config.
Much faster than full 182-sample eval.
"""
...
def main():
    ...
    # Test configs: vary max_rounds
    configs = [
        {'name': '2R', 'rounds': 2, 'c_iters': 300, 'f_iters': 300, 'c_lr': 0.01, 'f_lr': 0.005},
        {'name': '3R', 'rounds': 3, 'c_iters': 300, 'f_iters': 300, 'c_lr': 0.01, 'f_lr': 0.005},
        {'name': '4R', 'rounds': 4, 'c_iters': 300, 'f_iters': 300, 'c_lr': 0.01, 'f_lr': 0.005},
        {'name': '3R_c500', 'rounds': 3, 'c_iters': 500, 'f_iters': 300, 'c_lr': 0.01, 'f_lr': 0.005},
        {'name': '3R_nolrdecay', 'rounds': 3, 'c_iters': 300, 'f_iters': 300, 'c_lr': 0.01, 'f_lr': 0.005, 'no_lr_decay': True},
    ]
    ...
    for si, idx in enumerate(tqdm(sample_indices, desc="Samples")):
        ...
        if (si + 1) % 5 == 0:
            ...
            for name in results:
                p = results[name]['pos']
                r = results[name]['rot']
                print(f"    {name:15s}: med_pos={np.median(p):.1f}cm  med_rot={np.median(r):.2f}°")
    
    print("\n" + "="*70)
    print(f"RESULTS on {n_samples} samples (paper target: 21cm / 0.41°)")
    print("="*70)
    for name in results:
        p = results[name]['pos']
        r = results[name]['rot']
        print(f"  {name:15s}: med_pos={np.median(p):6.1f}cm  med_rot={np.median(r):.2f}°  "
              f"mean_pos={np.mean(p):6.1f}cm  mean_rot={np.mean(r):.2f}°  "
              f"p90_pos={np.percentile(p, 90):6.1f}cm")
